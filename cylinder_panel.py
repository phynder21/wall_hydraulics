...
def render_cylinder_sizing(force_per_kg, mass, key_prefix="", imperial=None):
    ...
    keys unique across views (Designer uses '', Browse uses 'lk_'). `imperial` ties
    the bore standard and pressure unit to the app's Imperial/Metric switch —
    True → NFPA inch bore + psi, False → ISO metric bore + bar; pass None to show an
    independent bore-standard selector (Browse/Reverse). Returns the resolved
    (design pressure in bar, bore-standard string)."""
        ...
        if imperial is None:
            ...
            series = cs1.selectbox(
                ...
                help="Round the required bore up to a real catalog size, or show the "
                     "exact number.")
            use_psi, pcol = series == "NFPA (inch)", cs2
        else:
            # Driven by the master Imperial/Metric toggle: fixed standard, no selector.
            series = "NFPA (inch)" if imperial else "ISO metric"
            use_psi, pcol = imperial, st
        if use_psi:
            st.session_state.setdefault(f"{key_prefix}des_psi", 3000.0)
            press_psi = pcol.number_input("Design pressure (psi)", 200.0, 10000.0,
            ...
            pressure_bar = pcol.number_input("Design pressure (bar)", 20.0, 700.0,
