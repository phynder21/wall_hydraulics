...
# The sensitivity charts color by peak force. `arr` indexes the per-variable profile
# tuple (values, forces, lengths, ok); force is per cylinder, so it divides by the
# cylinder count. Lower force is better, so blue = better / red = worse.
_FORCE = dict(arr=1, key_label="Force vs.<br>current", word="peak force",
...
def _ratio_colorbar(dtick, key_label="Force vs.<br>current"):
    ...
    `key_label` names what is colored (peak force)."""
...
def _current_force(a, b, d, f, x_cg, z_cg):
    """The current design's peak force — the 100% reference the strip and map color
    relative to."""
    peak, _, _ = peak_force_feasible(a, b, d, f, x_cg, z_cg)
    return peak
...
def build_sensitivity_figures(a, b, d, f, x_cg, z_cg, bounds, n_cyl=1,
    ...
    rejected. The charts color by peak force. Returns (fig_bar, fig_strip, caption)."""
    M = _FORCE
    ai = M["arr"]                                        # profile-tuple index of force
    div = n_cyl                                          # force is per cylinder
    ...
    percyl = " per cyl" if n_cyl > 1 else ""
    ...
    # there and the force becomes 150% of what it is now." 100% (white) =
    ...
    m0 = _current_force(a, b, d, f, x_cg, z_cg)          # current value = 100%
...
def render_sensitivity_panel(a, b, d, f, x_cg, z_cg, bounds, n_cyl=1,
    ...
    The charts color by peak force. Returns (fig_bar, fig_strip) so the caller can
    embed the same charts in the PDF."""
    with st.container(border=True):
        ...
        st.markdown(f"**Sensitivity — which dimension moves the {_FORCE['short']} "
                    f"most?**")
...
def _pair_grid(v1, v2, a, b, d, f, x_cg, z_cg, r1, r2, res, feas):
    ...
    for i, yv in enumerate(vals2):
        for j, xv in enumerate(vals1):
            p, _lm, fe = peak_force_feasible(**{**base, v1: float(xv), v2: float(yv)},
...
def render_interaction_map(a, b, d, f, x_cg, z_cg, bounds, n_cyl=1,
    ...
    strip (% of the current peak force; black = a spot the optimizer would reject), so
    it exposes the interactions the one-at-a-time strip can't see (combined moves)."""
    M = _FORCE
    div = n_cyl                                          # force is per cylinder
    with st.container(border=True):
        ...
        grid = peak                                       # color by peak force
        m0 = _current_force(a, b, d, f, x_cg, z_cg)        # current = 100%
...
def build_interaction_matrix(a, b, d, f, x_cg, z_cg, bounds, res=41, stroke_max=None,
    ...
    a, b, d, f), on ONE shared 'peak force vs. current %' scale so the panels compare.
    ...
    m0 = _current_force(a, b, d, f, x_cg, z_cg)          # shared ref = current
